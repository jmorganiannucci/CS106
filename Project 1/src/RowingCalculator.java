...
public class RowingCalculator {
	...
	public static void calculateSplitAverage(Scanner myScanner){
		...
		if (splitInSecs == 0){ //if split in seconds is equal to a value of zero then a second zero also needs to be printed 
			System.out.println(splitInMins +":"+ splitInSecs +"0"); //print statement that adds on additional zero 
		}	
		else if(splitInSecs < 10){ //unless the seconds value is going to be less than 10 (09,08,07...) skip first if, there needs to be a zero in front of the single printed number (2:9 -> 2:09)
			System.out.println(splitInMins +":0"+ splitInSecs); 
			
		}
		else{ //if the pervious if and else if statements are not true they will skip and print regularly without zeros in the following print statement 
	...
	public static void calculateTime (Scanner myScanner){
		...
		int totalTimeRem = (totalTimeInSec % 60); 
